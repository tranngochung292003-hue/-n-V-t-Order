...
<script type="module">
  ...
  // DÁN ĐÚNG THÔNG TIN Firebase Web App CỦA BẠN VÀO ĐÂY.
  // VAPID key đã được cấu hình theo key bạn cung cấp.
  ...
  window.initFirebaseNotifications = async function () {
    try {
      if (!("Notification" in window)) return {success:false, message:"Thiết bị không hỗ trợ thông báo."};

      const cfg = window.FIREBASE_WEB_CONFIG || {};
      const required = ["apiKey","authDomain","projectId","storageBucket","messagingSenderId","appId"];
      if (required.some(k => !cfg[k])) {
        return {success:false, message:"Chưa nhập đủ Firebase Web App config."};
      }

      if (!(await isSupported())) {
        return {success:false, message:"Trình duyệt này không hỗ trợ Firebase Web Push."};
      }

      const permission = await Notification.requestPermission();
      if (permission !== "granted") {
        return {success:false, message:"Bạn chưa cho phép hiển thị thông báo."};
      }

      // Đăng ký đúng Service Worker ở thư mục gốc.
      // File này phải tồn tại tại /firebase-messaging-sw.js
      ...
      if (token) {
        ...
        // Gửi token về Google Apps Script ngay khi người dùng cấp quyền,
        // không cần chờ khách đặt đơn.
        try {
          ...
          console.warn("Không gửi được FCM token về Google Apps Script:", saveError);
        }

        return {success:true, token};
      }

      return {success:false, message:"Không lấy được FCM token."};
    } catch (error) {
      console.warn("Firebase notification:", error);
      return {success:false, message:error.message || "Không thể bật thông báo."};
    }
  };
</script>
...
async function enableNotifications(){
  ...
  try{
    if(!("Notification" in window)){
      alert("❌ Thiết bị/trình duyệt này không hỗ trợ thông báo.");
      return;
    }

    const result = window.initFirebaseNotifications
      ? await window.initFirebaseNotifications()
      : {success:false, message:"Firebase chưa sẵn sàng."};

    if(result.success){
      localStorage.setItem("ANVAT_NOTIFICATION_ENABLED","1");
      updateNotificationPermissionUI();
      alert("✅ Đã bật thông báo.\n\nWebsite có thể lưu mã thông báo để gửi cập nhật đơn hàng.");
      return;
    }

    // Fallback: vẫn xin quyền Notification API để website có thể hiển thị
    // thông báo cục bộ khi trang đang mở.
    const permission = await Notification.requestPermission();

    if(permission === "granted"){
      localStorage.setItem("ANVAT_NOTIFICATION_ENABLED","1");
      updateNotificationPermissionUI();
      new Notification("🔔 Ăn Vặt Lâm Thao", {
        body:"Đã bật thông báo thành công.",
        icon:"icons/icon-192.png"
      });
    }else{
      updateNotificationPermissionUI();
      alert("⚠️ Chưa bật được thông báo.\n\n" + (result.message || "Vui lòng kiểm tra quyền thông báo của trình duyệt."));
    }
  }catch(error){
    console.warn("enableNotifications:", error);
    updateNotificationPermissionUI();
